...
class ConditionCollection(object):
    ...
    def __pbar__(self):
        if len(self.contents) >= self.__pbar_min_num__:
            stck = inspect.stack()
            pbar = progressbar.ProgressBar(widgets=[progressbar.widgets.Counter(), progressbar.widgets.RotatingMarker()])
            if len(stck) > 1:
                t_name = stck[1][3]
                pbar.widgets.insert(0, progressbar.widgets.FormatLabel(t_name + '--> '))
            return pbar
        return iter
