...
def submatrix(chief_res, gid_collection, M, info):
    """
...
def count_triads_all(tribes, M, info, cfg, **kwargs):
    ...
    lst_results = []
    for chief_res in tribal_chiefs.contents:
        print("Counting triads for: {0}".format(chief_res.cond))
        subM, chief_idx = submatrix(chief_res, tribal_gids, M, info)
        ...
        lst_results.append(ResultsWithConditions(res, **chief_res.cond))
