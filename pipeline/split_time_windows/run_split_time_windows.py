...
def execute_split(spikes, stimuli, data_cfg):
    ...
    t_bins = numpy.arange(0, numpy.max(spikes[:, 0]) + splt_t, splt_t)
    ...
    for i, stim in enumerate(stimuli):
        curr_t_start = i * data_cfg["stim_duration_ms"]
        curr_t_end = curr_t_start + data_cfg["stim_duration_ms"]
        win = spikes[t_bin_idx == i]
        if len(win) == 0:
            print("Warning: no spikes between {0} and {1} ms".format(curr_t_start, curr_t_end))
        win[:, 0] = win[:, 0] - curr_t_start
